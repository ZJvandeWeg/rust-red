...
async fn can_create_flow_manually() {
    ...
    let flow = engine.get_flow(&"dee0d1b0cfd62a6c".parse().unwrap()).unwrap();
    let inject_node = flow.get_node(&"bf843d35fe7cf583".parse().unwrap()).unwrap();
    assert_eq!(inject_node.state().type_name, "inject");
